...
export default function TabsMenuIG(props){
    var base = (props.search !== undefined && props.search !== null && props.search !== "") ? "/interview-guide/":""
    return (
        ...
        <TabList>
            {
                IGTabsNames.map((tabName) => 
                    <Link key={"link-" + tabName} href={base + tabName.replace(/\s/g, '').replace(/&/g, '').toLowerCase() + "/" + props.search} passHref>
